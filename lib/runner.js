...
Runner.prototype.run = function(fn) {
    ...
    self.emit('start')
    self.all_tests = []
    final_node = self._setup_dependencies(root_node, this.suite)
...
Runner.prototype._setup_dependencies = function (root_node, suite) {
    var self = this
      , suite_queue = [suite]
      , suite_level = 0
      , final_suites = []
      , next_suite_queue = []
      , final_runnable = new Runnable('Final node', function () {})
    ...
    final_runnable.add_dependencies(self._get_edges())
    final_runnable._dependencies.forEach(function (pre_test) {
        console.log('"' + final_runnable.fullTitle() + '"','->','"'+pre_test.fullTitle()+'";')
    })
...
Runner.prototype._walk_tests = function (root_node, suite_graph) {
    ...
    for (var i = 0; i < max_tests; i++) {
        ...
        suite_graph.forEach(function (suite) {
            ...
            //console.log(test.title, '<-', test.dependency)
        ...
        exclusives.forEach(function (test) {
            ...
            //console.log('ex', test.title, '<-', test.dependency)
            test._dependencies.forEach(function (pre_test) {
                console.log('"' + test.fullTitle() + '"','->','"'+pre_test.fullTitle()+'";')
            })
        })
...
Runner.prototype._setup_test_hooks = function (test) {
    ...
    test.on('started', function () {
        self.emit('test', test)
        //console.log('*********** STARTING', test.fullTitle())
    })

    test.on('completed', function () {
        //console.log('*********** COMPLETE', test.fullTitle())
        if (test.err) {
