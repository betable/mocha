...
Runner.prototype.run = function(fn) {
    ...
    // run suites
    self.emit('start')
    self.suite.root = true
    root_node.exclusive(true)
    ...
    final_node.exclusive(true)
    final_node.root_node = true
...
Runner.prototype._setup_dependencies = function (root_node, suite_queue) {
    ...
    do {
        ...
        self._walk_tests(root_node, final_suites[suite_level], '_beforeAll')
        self._walk_tests(root_node, final_suites[suite_level], 'tests')
        self._walk_tests(root_node, final_suites[suite_level], '_afterAll')
    ...
    exclusives.forEach(function (exclusive_suite) {
        var wait_node = new Runnable('Wait node for ' + exclusive_suite.fullTitle(), function () {})
        wait_node.exclusive(true)
        wait_node.root_node = true
        wait_node.add_dependencies(self._get_edges([wait_node]))
        self.all_tests.push(wait_node)

        self._walk_tests(wait_node, [exclusive_suite], '_beforeAll')
        self._walk_tests(wait_node, [exclusive_suite], 'tests')
        self._walk_tests(wait_node, [exclusive_suite], '_afterAll')
        self._setup_dependencies(wait_node, exclusive_suite.suites)

        root_node = new Runnable('Final node for ' + exclusive_suite.fullTitle(), function () {})
        root_node.exclusive(true)
        root_node.root_node = true
...
Runner.prototype._walk_tests = function (root_node, suite_graph, test_type) {
    var self = this
      , max_tests = 0
      , exclusives = []

    suite_graph.forEach(function (suite) {
        max_tests = Math.max(max_tests, suite[test_type].length)
    })
    ...
    for (var i = 0; i < max_tests; i++) {
        ...
        suite_graph.forEach(function (suite) {
            var test = suite[test_type][i]
              , next_dependency = [root_node]

            if (!test) {
                return
            }

            test.parents().some(function (parent) {
                if (parent.next_dependency) {
                    next_dependency = parent.next_dependency
                    return true
                }
                return false
            })
            ...
            if (test.exclusive()) {
                ...
                test.add_dependencies(next_dependency)
        ...
        exclusives.forEach(function (test) {
            ...
            self.all_tests.push(test)
            self._get_edges([test])
        })
    }
}

Runner.prototype._get_edges = function (new_root) {
    var edges = []

    //TODO: We can do better than brute forcing it
    this.all_tests.forEach(function (test) {
        //TODO: Make sure we don't have to actual do this :(
        /*
        if (new_root) {
            test.parents().forEach(function (parent) {
                parent.next_dependency = new_root
            })
        }
        */

        //TODO: I think this works better than above, only resets the first parent of all nodes
        if (new_root && test.parent) {
            test.parent.next_dependency = new_root
        }
...
Runner.prototype.print_dot_graph = function () {
    var node_id = 1
      , color = ''
      , fd = fs.openSync('/Users/nate/graphviz/test.dot', 'w', 0755)
      , line = ''
      , subgraph_id = 0
      , subgraphs = []

    fs.writeSync(fd, 'digraph G\n', null, 'utf8')
    fs.writeSync(fd, '{\n', null, 'utf8')
    ...
    this.all_tests.forEach(function (test) {
        ...
        line = test.node_id + ' [shape=rectangle, label="' + test.title + '", style=filled, fillcolor=' + color + '];'
        fs.writeSync(fd, line + '\n', null, 'utf8')

        if (test.parent) {
            if (!test.parent.subgraph_id) {
                test.parent.subgraph_id = subgraph_id++
                subgraphs[test.parent.subgraph_id] = {
                    id: test.parent.subgraph_id
                  , label: test.parent.fullTitle('/')
                  , tests: []
                }
            }
            
            subgraphs[test.parent.subgraph_id].tests.push(test)
        }
    })
    ...
    subgraphs.forEach(function (subgraph) {
        ...
        line += '    graph[style=dotted, labelloc=b];\n'

        fs.writeSync(fd, line, null, 'utf8')

        subgraph.tests.forEach(function (test) {
            fs.writeSync(fd, '    ' + test.node_id + ';\n', null, 'utf8')
        })

        fs.writeSync(fd, '}\n', null, 'utf8')
    })

    /* Gives a subgraph heirarchy wait nodes and final nodes are weird though
    node_id = 0;
    function build_clusters (suite, prefix) {
        line = prefix + 'subgraph cluster_' + node_id + ' {\n'
        line += prefix + '    label="' + suite.title + '";\n'

        if (suite.root) {
            line += prefix + '    graph[style=invis];\n'
        } else {
            line += prefix + '    graph[style=dotted, labelloc=b];\n'
        }

        fs.writeSync(fd, line, null, 'utf8')
        node_id++

        suite._beforeAll.forEach(function (test) {
            fs.writeSync(fd, prefix + '    ' + test.node_id + ';\n', null, 'utf8')
        })
        ...
        suite._afterAll.forEach(function (test) {
            fs.writeSync(fd, prefix + '    ' + test.node_id + ';\n', null, 'utf8')
        })

        suite.suites.forEach(function (suite) { build_clusters(suite, prefix + '    ') })
        fs.writeSync(fd, prefix + '}\n', null, 'utf8')
    }
    build_clusters(this.suite, '')
    */

    this.all_tests.forEach(function (test) {
        test._dependencies.forEach(function (pre_test) {
            line = test.node_id + ' -> ' + pre_test.node_id + ';'
            fs.writeSync(fd, line + '\n', null, 'utf8')
        })
    })
