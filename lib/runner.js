...
Runner.prototype.run = function(fn) {
    var self = this
      , root_node = new RootNode('Root', function () {})
      , final_node = new RootNode('Final node', function () {})
...
Runner.prototype._setup_dependencies = function (root_node, suite_queue) {
    ...
    do {
        ...
        while (suite_queue.length > 0) {
            ...
            if (sub_suite.exclusive()) {
                ...
                suite_level.push(sub_suite)
                next_suite_queue.push.apply(next_suite_queue, sub_suite.suites)
            }
        }

        //TODO: Move the all test calculation to the suite and make this less stupid
        self._walk_tests(root_node, suite_level, '_beforeAll')
        self._walk_tests(root_node, suite_level, 'tests')
        self._walk_tests(root_node, suite_level, '_afterAll')
    ...
    exclusives.forEach(function (exclusive_suite) {
        var wait_node = new RootNode('Wait node for ' + exclusive_suite.fullTitle(), function () {})
        ...
        root_node = new RootNode('Final node for ' + exclusive_suite.fullTitle(), function () {})
