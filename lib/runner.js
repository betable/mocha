...
function Runner(suite) {
    ...
    /*process.on('SIGINT', function () {
        //TODO: make this more graceful and expose end and do this in mocha.js or bin/_mocha
...
/**
 * Run the root suite and invoke `callback(failures)` on completion.
 *
 * @param {function(Number)} callback A function that is called once the runner has completed
 *
 * @return {Runner} for chaining
 */
Runner.prototype.run = function(callback) {
    ...
    final_node.on('completed', function () {
        try {
            //TODO: Need to grab domain.on('error', ...) here
            self.emit('end')
...
Runner.prototype._setup_dependencies = function (root_node, suite_queue) {
    ...
    do {
        ...
        while (suite_queue.length > 0) {
            ...
            if (sub_suite.exclusive()) {
                ...
                next_suite_queue = next_suite_queue.concat(sub_suite.suites)
            }
        }
    ...
    exclusives.forEach(function (exclusive_suite) {
        var wait_node = new RootNode('Wait node for ' + exclusive_suite.full_title(), function () {})
