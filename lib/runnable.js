...
/**
 * Initialize a new Runnable with the given title and test function
 * Runnable is a base class to all runnable types (test, suite hooks, test hooks)
 *
...
var Runnable = function (title, test_function) {
    ...
    this.errors = []
    this.state = Runnable.STATE_WAITING
...
Runnable.STATE_BEFORE_HOOKS = 2
Runnable.STATE_RUNNING = 3
Runnable.STATE_AFTER_HOOKS = 4
Runnable.STATE_COMPLETED = 5
 ...
 * @api public
 */
Runnable.prototype.full_title = function (separator) {
    ...
    separator = separator || ' '
    return this.parent.full_title(separator) + separator + title
}
...
Runnable.prototype.reset_timeout = function () {
    ...
    if (ms) {
        this.timer = setTimeout(function(){
            self._complete(new Error('timeout of ' + ms + 'ms exceeded'))
            self.timed_out = true
        }, ms)
    }
}
...
/**
 * Actually runs the test function registered for this object
 *
 * @param {function} callback A function to call when the test function is complete
 *
 * @private
 */
Runnable.prototype._run = function (callback) {
    var self = this
    ...
    process.nextTick(function () {
        self.domain.run(function () {
            try {
                ...
                var return_values = self.test_function.call(
                    context
                  , function (error) {
                        self._complete(error)
                    }
 ...
 * @private
 */
Runnable.prototype._complete = function (error) {
    var self = this
    ...
    if (self.state === Runnable.STATE_COMPLETED) {
        if (self.timed_out) {
            return
        }
