...
function XUnitFile(runner) {
    ...
    runner.on('test pass', function (test) {
        process.stdout.write('Passed: ' + test.full_title() + '\n')
        tests.push(test)
    })

    runner.on('test fail', function (test) {
        process.stdout.write('Failed: ' + test.full_title() + '\n')
        tests.push(test)
    })

    runner.on('test skip', function (test) {
        process.stdout.write('Skipped: ' + test.full_title() + '\n')
        tests.push(test)
    })
    ...
    runner.on('end', function () {
        self.failures.forEach(function (test, i) {
            ...
            console.error((i + 1) + ') ' + test.full_title())
